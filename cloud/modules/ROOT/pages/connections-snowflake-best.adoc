...
You can choose to make a connection to access _all_ your Snowflake databases, or to one specific database. If you only want to access one {connection} database, you can specify the database in the *Database* text box in *Specify connection details* during setup.

To allow ThoughtSpot to connect only to a specific {connection} database, follow these steps:
...
. For the specific database you would like to access, set `database` as the key, and the database name as the value.
