...
. Modify the answer, as needed, by adding columns using the list on the left or by typing the column name next to existing search tokens, which appear next to the name of the data set.
. When you are satisfied with the answer, click *Next*.
+
[.bordered]
image::spotter-coach-3.png[]
. Review the business terms used for the search phrase you typed and their mapped search tokens.
...
. To ask more questions, click *Ask another question*.
