...
ifdef::spotter[]
. Click the app switcher menu image:spotter-app-switcher.png[] and then click *Spotter*.
. Select *Manage data > Add data source* on the left side of the screen.
. On the _Add a data source_ page, click *Connect data*.
+
[.bordered]
image::connection-spotter-set-up-new-connection.png[]
endif::[]
...
+
[.bordered]
image::connection-spotter-set-up-new-connection-3.png[model created]

== Coach Spotter

You can teach Spotter how your organization uses your data by asking some questions, viewing the answers and refining them through your feedback. This helps Spotter to provide the kinds of answers you are looking for based on how you ask your questions. You can skip this step by clicking *Skip*. To coach Spotter later, go to *Manage data > Coach Spotter*, then click *Add coaching*.

To coach Spotter, do the following:

. Click *Coach Spotter*.
. Select your data set, if it is not already selected.
. In the field to the right of the data set name, enter a business question and click *Next*.
+
[.bordered]
...
. Modify the answer, as needed, by adding columns using the list on the left, or by entering the column name next to the existing search tokens in the search field next to the name of the data set.
...
Now you are ready to use Spotter with your data set.

. Click *Chat*.
. On the _Spotter_ page, enter a question in the _Ask a question_ field, and click the submit button image:spotter-submit.png[submit chat button,width="40px"].
+
[.bordered]
image::spotter-chat-box.png[chat box]
. Edit the answers, if needed, by clicking the *Edit* link next to the search tokens of the Spotter answer.
+
[.bordered]
