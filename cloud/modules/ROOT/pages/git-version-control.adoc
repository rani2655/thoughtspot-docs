...
== Version control setup for an Org
...
.. For _Token_, enter or paste the personal access token for the repository.
+
...
. Click *Enable*.
+
When the configuration complete and version control is enabled, the following message appears at the bottom of Version Control page:
+
image::git-finished.png[]
. Click *Refresh now*.
