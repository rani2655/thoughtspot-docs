...
You can now drill down on the results of change analysis to learn more about change drivers in your data. To run iterative change analysis, follow these steps:

. Select two points on a KPI chart and right-click to select *Run change analysis*.

. Right-click on a column in the resulting change analysis and select *Identify change drivers for <column_name>*.

. ThoughtSpot adds the specified column as a filter and re-calculates change analysis for the original comparison points specified in your search.
