...
NOTE: Users can now interact with filters on Liveboards containing visualizations that include hidden columns, or formulas created from hidden columns.

NOTE: If you want to make a certain column available only to a subset of users, we recommend using xref:security-data-object.adoc#cls[column-level security].
