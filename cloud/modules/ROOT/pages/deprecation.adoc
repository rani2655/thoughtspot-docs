...
The 9.5.0.cl release of ThoughtSpot Cloud, targeted for August 2023 (GA), will remove the following feature:

Automatically select my chart toggle:: ThoughtSpot is removing the *Automatically select my chart* toggle at the bottom of the chart type selection panel. Once you select a chart type, ThoughtSpot retains the chart type as you make changes to the search query. For cases where the search query can no longer render as the same chart type (due to the removal of necessary attributes or measures), ThoughtSpot automatically selects a chart type that can render based on the query.

== Deprecated in 9.4.0.cl

The 9.4.0.cl release of ThoughtSpot Cloud, targeted for August 2023 (GA), will drop support for the following feature:
