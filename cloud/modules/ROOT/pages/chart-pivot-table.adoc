...
.Bleneded Axes [.badge.badge-beta]#Beta#
****
To compare multiple measures in a single view you can now blend two measures to share an axis. Blending measures uses the *Measure names* and *Measure values* fields to group and control multiple measures in pivot table charts. Measure names and measure values can be included in charts using these augmented columns. You can move measures and position them where you want in rows and columns. You can configure where the measures appear in the table with respect to other attributes being used in either rows or columns.

NOTE: Measures are placed before attributes in rows.
