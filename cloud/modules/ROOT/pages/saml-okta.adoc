...
To enable local authentication at the cluster level, administrators require the *Can manage authentication* privilege. To enable local authentication at the Org level, contact {support-url}. Org level settings override cluster level settings.
...
By default, local authentication is enabled for new Orgs. Local authentication is also visible in the *All Orgs* section. Disabling local authentication only disable it for the cluster URL if per-Org URLs are configured.
