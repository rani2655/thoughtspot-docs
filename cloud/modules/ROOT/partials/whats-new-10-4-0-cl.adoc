...
// PM: Damian. confirm if beta. confirm if enabling or disabling alphabetic sorting will turn A-Z sorting back on. get a screenshot with custom group columns (named custom column group) with emphasis box.

Analysts can now set definitions in a Worksheet or Model's TML file to group columns together in the Data panel for Search Data. You can also use TML properties to enable or disable alphabetical sorting of the groups. To enable this feature, contact {support-url}.
...
// PM: Aashica. reword last sentence.
