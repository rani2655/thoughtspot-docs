...
// PM: Dilip. clarify which parts are actually new, make clear it's only in the new homepage experience. make it clear you no longer need liveboard schedule privileges. can they remove themselves from a schedule someone else set up?
...
// PM: Dilip
//Dilip confirmed that there are no additional customer facing changes made as part of this in 10.4 (back end).
////

[#10-4-0-cl-sync]
[discrete]
=== ThoughtSpot Sync: send full image of Liveboard to Slack
ThoughtSpot introduces the ability for users to select whether they want to export a complete Liveboard as a PDF or a PNG image of the first page when creating a new sync pipeline.
...
// PM: Dilip. No doc needed per Dilip.
////
