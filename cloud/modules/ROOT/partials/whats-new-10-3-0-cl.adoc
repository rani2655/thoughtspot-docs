...
In this release, administrators can hide date previews in a date filter which uses a custom calendar. This ensures users are not confused if the values are incorrect, which can happen sometimes when using a custom calendar with a date filter. You can hide date previews in the following situations:
