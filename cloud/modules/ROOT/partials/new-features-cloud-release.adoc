...
xref:studio-get-started.adoc[Analyst Studio].
...
Content-aware filter values for Sage Coach:: We made the following improvements to filters in Sage Coach: +

* The only values displayed in the Worksheets filter dropdown are Worksheets for which feedback is applied.
* The only values displayed in the users filter dropdown are users who have given feedback.
