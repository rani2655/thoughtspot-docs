...
// Mary -- SCAL-158899
// PM: Damian - Moved from 9.4.0.cl BETA, updated terminology, needs new .gif as the UI has changed slightly.
...
ThoughtSpot allows users to export a Liveboard as CSV. By default, the exported CSV has some additional information, such as filters applied, and user information. ThoughtSpot is introducing the ability to remove this additional information when exporting a Liveboard as a CSV.
By default, this feature will be turned off. To enable this feautre, contact {support-url}.
// Mary -- SCAL-179229
...
ThoughtSpot introduces support for custom URL links to scheduled Liveboard emails. If users have set a custom URL for their clusters, the links in the scheduled emails will now include the custom URL.
// Mary -- SCAL-168830
// PM: Vanshree
